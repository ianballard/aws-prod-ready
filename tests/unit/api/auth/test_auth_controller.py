...
@patch("api.auth.auth_controller.auth_service.sign_up")
@patch("api.auth.auth_controller.generate_uuid")
def test_signup(generate_uuid, sign_up, create_user):
    username = "User01"
    ...
    sign_up.return_value = expected_signup_return_value
    ...
    sign_up.assert_called_once_with(**sign_up_input)
...
@patch("api.auth.auth_controller.auth_service.confirm_sign_up")
def test_confirm_signup(confirm_sign_up):
    event = {
        "headers": {},
        "pathParameters": {},
    ...
    confirm_sign_up.assert_called_once_with(**event["queryStringParameters"])


@patch("api.auth.auth_controller.auth_service.respond_to_new_password_auth_challenge")
def test_new_password_auth_challenge(respond_to_new_password_auth_challenge):
    event = {
        "headers": {},
        "pathParameters": {},
        "body": json.dumps(
            {"username": "User01", "password": "Password@123", "session": "session_id"}
        ),
        "queryStringParameters": {},
    }
    ...
    respond_to_new_password_auth_challenge.return_value = (
        expected_auth_service_return_value
    )

    sign_up_response = auth_controller.new_password_auth_challenge(event, None)
    ...
    respond_to_new_password_auth_challenge.assert_called_once_with(
