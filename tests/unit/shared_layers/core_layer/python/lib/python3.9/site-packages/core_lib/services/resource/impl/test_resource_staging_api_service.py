...
@patch(
    "core_lib.services.resource.impl.resource_staging_api_service.resource_staging_api_client"
)
@patch(
    "core_lib.services.resource.impl.resource_staging_api_service.get_non_null_kwargs"
)
def test_get_resources(mock_get_non_null_kwargs, mock_resource_staging_api_client):
    ...
    # Assert
    assert result == {"pagination_token": "test", "resource_arns": test_resource_arns}
...
def test_get_lambda_resources_by_function_role(mock_get_resources):
    ...
    mock_get_resources.return_value = {
        "pagination_token": "test",
        "resource_arns": ["arn1", "arn2"],
    }

    # Act
    result = resource_staging_api_service.get_lambda_resources_by_function_role(
        function_role_tag_value=test_function_role_tag_value, pagination_token="test"
    )

    # Assert
    assert result == {"pagination_token": "test", "resource_arns": ["arn1", "arn2"]}
