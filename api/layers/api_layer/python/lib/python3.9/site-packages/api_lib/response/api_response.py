...
class ApiResponse:
    ...
    def format(self):
        ...
        formatted_response["body"] = (
            json.dumps(self.body, cls=json.JSONEncoder) if self.body is not None else None
        )
...
def set_access_control_allow_origin(request_headers: dict = None):
    ...
    if (request_origin is not None and request_origin in CORS_ALLOWED_ORIGINS) or CORS_ALLOWED_ORIGINS == ['*']:
