...
from core_lib.services.auth.impl.cognito_service import get_user_pool_id, get_user_pool_client
...
def get_decoded_jwt(
    token: str, is_access_token=False, check_exp_claim=True
):
    ...
    verified_token = jwt.decode(token, public_key_pem, algorithms=['RS256'], audience=USER_POOL_CLIENT, options={"verify_signature": True})
