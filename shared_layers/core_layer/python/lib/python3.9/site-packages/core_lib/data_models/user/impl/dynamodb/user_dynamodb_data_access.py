...
def find_user_by_id(_id: str, projection_expression: str = None):
    return get_db_item(
        Key=build_user_item_key(_id=_id), ProjectionExpression=projection_expression
    )
...
def create_user(user):
    return put_db_item(Item=validate_user(user=build_new_user(user)))


def update_user(_id: str, updates: dict):
    validate_user_updates(updates=updates)
...
def associate_users(user_a_id: str, user_b_id):
    put_db_item(Item=build_user_association(user_a_id=user_a_id, user_b_id=user_b_id))
