...
from core_lib.data_models.user.user_data_model import USER_ENTITY_TYPE
...
def build_new_user(user):
    ...
    user["sk"] = f"{USER_ENTITY_TYPE.value}-{str(uuid4())}"
...
def query_users():
    return query_db_items(KeyConditionExpression=Key("pk").eq(USER_ENTITY_TYPE.value))
