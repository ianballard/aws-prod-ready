...
def db_table():
    ...
    if DB_TABLE is None:
        DB_TABLE = dynamodb_resource.Table(os.getenv("DB_TABLE"))
...
def soft_delete_db_item(**kwargs):
    ...
    db_response = db_table().update_item(
        Key=kwargs.get("Key"),
...
def batch_get_item(**kwargs):
    db_response = dynamodb_resource.batch_get_item(**get_non_null_kwargs(**kwargs))
    return db_response.get("Responses", {}).get(os.getenv("DB_TABLE"), [])
