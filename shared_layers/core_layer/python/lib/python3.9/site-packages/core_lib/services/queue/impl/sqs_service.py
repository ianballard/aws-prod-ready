...
def get_access_log_queue_url():
    ...
    if ACCESS_LOG_QUEUE_URL is None:
        parameter_name = (
            ParameterName.ACCESS_LOG_QUEUE_URL_VPC
            if is_vpc_enabled
            else ParameterName.ACCESS_LOG_QUEUE_URL_NO_VPC
        )
...
def get_error_log_queue_url():
    ...
    if ERROR_LOG_QUEUE_URL is None:
        parameter_name = (
            ParameterName.ERROR_LOG_QUEUE_URL_VPC
            if is_vpc_enabled
            else ParameterName.ERROR_LOG_QUEUE_URL_NO_VPC
        )
...
def get_auth_event_queue_url(region: str = os.getenv("region")):
    ...
    if ERROR_LOG_QUEUE_URL is None:
        parameter_name = (
            ParameterName.AUTH_EVENT_QUEUE_URL_VPC
            if is_vpc_enabled
            else ParameterName.AUTH_EVENT_QUEUE_URL_NO_VPC
        )
        ERROR_LOG_QUEUE_URL = get_parameter_value(
